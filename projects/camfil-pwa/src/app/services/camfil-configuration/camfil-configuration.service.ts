...
import { getCamfilSettings } from 'camfil-pwa/store/camfil-configuration';
...
export class CamfilConfigurationService {
  ...
  private settings$: Observable<Partial<CamfilChannelSettings>>;
  ...
  private getCamfilConfigurationFromFile(channelName: string): Observable<ServerConfig> {
    return of(channelSettings).pipe(
      map(settings => ({
        data: { ...CamfilChannelConfigurationHelper.getSettingsByChannelName(settings, channelName) },
      })),
