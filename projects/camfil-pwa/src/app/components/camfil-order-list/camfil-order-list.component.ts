...
export class CamfilOrderListComponent implements OnInit, AfterViewInit, OnDestroy, OnChanges {
  ...
  dataSource = new MatTableDataSource<CamfilOrder>();
  isActive = false;
  showPreview = true;
  tableSize = 16;
  ...
  ngOnInit() {
    ...
    this.loading$ = this.camfilAccountFacade.ordersLoading$;
  ...
  orderFilterPredicate() {
    return (data: CamfilOrder, filterString: string): boolean => {
