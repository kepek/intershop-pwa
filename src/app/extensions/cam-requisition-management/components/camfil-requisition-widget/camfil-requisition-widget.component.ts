...
@Component({
  selector: 'camfil-requisition-widget',
  templateUrl: './camfil-requisition-widget.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
@GenerateLazyComponent()
export class CamfilRequisitionWidgetComponent implements OnInit {
  numPendingRequisitions$: Observable<number>;
  totalAmountRequisitons$: Observable<Price>;

  requisitionsLoading$: Observable<boolean>;

  constructor(private camrequisitionFacade: CamRequisitionManagementFacade) {}

  ngOnInit() {
    const pendingRequisitions$ = this.camrequisitionFacade.requisitions$('buyer', 'PENDING');
    ...
    this.requisitionsLoading$ = this.camrequisitionFacade.requisitionsLoading$;
  }
}
