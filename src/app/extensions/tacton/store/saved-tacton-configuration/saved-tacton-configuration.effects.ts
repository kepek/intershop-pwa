...
import { getExternalId, getTactonProductForSelectedProduct } from '../tacton-config';
  ...
  saveCurrentConfigurationReference$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        ...
        this.store.pipe(select(getExternalId))
      ),
      map(([configuration, tactonProduct, externalId]) =>
        saveTactonConfigurationReference({ configuration, tactonProduct, externalId })
      )
    )
  );
}
