  ...
  it("should get Product SKUs when 'getFilteredProducts' is called", done => {
    ...
    filterService.getFilteredProducts({ SearchParameter: ['b'] } as URLFormParams, 1).subscribe(data => {
      ...
      expect((params as AvailableOptions)?.params?.toString()).toMatchInlineSnapshot(
        `"amount=2&offset=0&attrs=sku,salePrice,listPrice,availability,manufacturer,image,minOrderQuantity,inStock,promotions,packingUnit,mastered,productMaster,productMasterSKU,roundedAverageRating,retailSet,stepQuantity&attributeGroup=PRODUCT_LIST_LABEL_ATTRIBUTES&returnSortKeys=true&SearchParameter=b"`
      );
